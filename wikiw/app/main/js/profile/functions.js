const L = (() => {
    ...
    C.prototype.attr = function (a, v) {
        ...
        this.e((i, j) => {
            if (L.type(a) === 'object') {
                L.each(a, (k, v) => {
                    // console.log(i);
                    i instanceof SVGElement ? i.setAttribute(k, v)
                                            : i[k] = v;
                });
                r = this;
            }
    ...
    C.prototype.swapClass = function (f, t, a = 1) {
        this.e((i) => {
            i.className.baseVal?"^"===f[0]?i.className.baseVal=~i.className.baseVal.indexOf(f.slice(1))?i.className.baseVal.replace(RegExp(f.slice(1)+"[^\\s]*","g"),t).trim():a?i.className.baseVal+" "+t:i.className.baseVal:"*"===f[0]?i.className.baseVal=~i.className.baseVal.indexOf(f.slice(1))?i.className.baseVal.replace(RegExp("[^\\s]*"+f.slice(1)+"[^\\s]*","g"),t).trim():a?i.className.baseVal+" "+t:i.className.baseVal:i.className.baseVal=~i.className.baseVal.indexOf(f)?i.className.baseVal.replace(f,t).trim():a?i.className.baseVal+" "+t:i.className.baseVal:"^"===f[0]?i.className=~i.className.indexOf(f.slice(1))?i.className.replace(RegExp(f.slice(1)+"[^\\s]*","g"),t).trim():a?i.className+" "+t:i.className:"*"===f[0]?i.className=~i.className.indexOf(f.slice(1))?i.className.replace(RegExp("[^\\s]*"+f.slice(1)+"[^\\s]*","g"),t).trim():a?i.className+" "+t:i.className:i.className=~i.className.indexOf(f)?i.className.replace(f,t).trim():a?i.className+" "+t:i.className;

// if (i.className.baseVal) {
// if (f[0]==='^') { // SELECT STRING THAT STARTS WITH
//       i.className.baseVal = ~i.className.baseVal.indexOf(f.slice(1)) ? i.className.baseVal.replace(RegExp(f.slice(1) + '[^\\s]*', 'g'), t).trim()
//                                                                      : a ? i.className.baseVal + ' ' + t : i.className.baseVal;
// }
// else if (f[0] === '*') { // SELECT WHOLE STRING THAT CONTAINS
//     i.className.baseVal = ~i.className.baseVal.indexOf(f.slice(1)) ? i.className.baseVal.replace(RegExp('[^\\s]*' + f.slice(1) + '[^\\s]*', 'g'), t).trim()
//                                                                    : a ? i.className.baseVal + ' ' + t : i.className.baseVal;
// }
// else {
//     i.className.baseVal = ~i.className.baseVal.indexOf(f) ? i.className.baseVal.replace(f, t).trim()
//                                                           : a ? i.className.baseVal + ' ' + t : i.className.baseVal;
// }
// }
// else {
//         if (f[0]==='^') {
//             i.className =  ~i.className.indexOf(f.slice(1)) ? i.className.replace(RegExp(f.slice(1) + '[^\\s]*', 'g'), t).trim()
//                                                             : a ? i.className + ' ' + t : i.className;
//         }
//         else if (f[0] === '*') {
//             i.className = ~i.className.indexOf(f.slice(1)) ? i.className.replace(RegExp('[^\\s]*' + f.slice(1) + '[^\\s]*', 'g'), t).trim()
//                                                            : a ? i.className + ' ' + t : i.className;
//         }
//         else {
//             i.className = ~i.className.indexOf(f) ? i.className.replace(f, t).trim()
//                                                   : a ? i.className + ' ' + t : i.className;
//         }
// }
        });
