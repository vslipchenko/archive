  ...
  class Auth {
    ...
    private $session;
      ...
      if (!is_numeric($this->session->get('id'))) {
        ...
        return $this->db->lastQueryResult();
      }
      else {
        die;
      }
  	...
  	public function login($email, $password) {
        if (!is_numeric($this->session->get('id'))) {
          ...
          if (password_verify($password, $result[0]['pass'])) {
            if ($result[0]['vcode']) {
              //send vcode on email
              sendMail($email, t['verification'], t['verificationCode'] . $result[0]['vcode']);
              return 2;
            }
            else {
  	...
  	public function verify(User $User, $email, $code) {
        ...
        $id = $this->db->query($query, $parameters)->fetchAll()[0]['id'];
        ...
        if ($id) {
          ...
          if (!is_numeric($this->db->query($query)->fetchAll()[0]['id'])) {
